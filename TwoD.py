...
class TwoD:
    def __init__(self, l_k_1, u_k_1, l_k_2, u_k_2, n_points=100, range=(0, 1), show_hard=True):
        ...
        self.show_hard = True  # Show what a hard interval would look like in this case
    ...
    def plot(self):
        ...
        if self.show_hard:
            surf = ax.plot_surface(self.X, self.Y, self.hard_interval(), color="b", label="hard interval")
        # surf = ax.plot_surface(self.X, self.Y, self.m_k(), label="soft interval")

        plt.title(f"Match probabilities for the classifier [{self.l_k_1}, {self.u_k_1}, {self.l_k_2}, {self.u_k_2}] "
    ...
    def hard_interval(self):
        ...
        for i, x in enumerate(self.x):
            for j, y in enumerate(self.y):
                res[i, j] = 1. if (self.l_k_1 < x < self.u_k_1 and self.l_k_2 < y < self.u_k_2) else 0.
        return res
